...
static PyObject* hough_transform(PyObject* self, PyObject* args, PyObject *keywds)
{
    ...
    theta_range = Py_None;
    int multi_vote = 0;
    static char *kwlist[] = {"image", "reference_image", "theta_range", "multi_vote", NULL};
	if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|OOp", kwlist, &image, &reference_image, &theta_range, &multi_vote))
	{
	...
	PyObject *thetas = PyArray_Arange(*(npy_float64 *) PyArray_GETPTR1(theta_range_array, 0), *(npy_float64 *) PyArray_GETPTR1(theta_range_array, 1), 1.0, NPY_FLOAT64);
	...
	PyObject *rhos = PyArray_Arange(-distance, distance + 1, 1.0, NPY_FLOAT64);
    ...
    if (multi_vote) {
        for (int i = 0; i < x_indexes_size; i++)
        {
            ...
            {
                int rho = (int) round(x * cos_table_data[theta_index] + y * sin_table_data[theta_index] + (npy_float64) distance);
                accumulator_data[(rho * thetas_size) + theta_index - 1] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
                accumulator_data[(rho * thetas_size) + theta_index] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
                accumulator_data[(rho * thetas_size) + theta_index + 1] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
            }
        }
    }
...
static PyObject* hough_transform_center(PyObject* self, PyObject* args, PyObject *keywds)
{
    PyObject *image, *theta_range, *reference_image;
    PyArrayObject *image_array, *theta_range_array, *reference_image_array;
    reference_image = Py_None;
	...
	int width = PyArray_DIM(image_array, 1);
	double distance = ceil(sqrt(width * width + height * height));
	PyObject *rhos = PyArray_Arange(-(distance / 2), (distance / 2) + 1, 1.0, NPY_FLOAT64);
	npy_intp accumulator_dims[] = {PyArray_Size(rhos), PyArray_Size(thetas)};
	PyObject *accumulator = PyArray_ZEROS(2, accumulator_dims, NPY_UINT64, 0);
	PyObject* zero = PyLong_FromLong(0);
	PyObject* equal = PyObject_RichCompare((PyObject *) image_array, zero, Py_EQ);
    ...
    if (multi_vote) {
        for (int i = 0; i < x_indexes_size; i++)
        {
            npy_int64 x = *(npy_int64 *) PyArray_GETPTR1((PyArrayObject *) x_indexes, i);
            npy_int64 y = *(npy_int64 *) PyArray_GETPTR1((PyArrayObject *) y_indexes, i);
            npy_float64 cx = (npy_float64) x - (npy_float64) (width / (npy_float64) 2.0);
            npy_float64 cy =  (npy_float64) y - (npy_float64) (height / (npy_float64) 2.0);
            for (int theta_index = 0; theta_index < thetas_size; theta_index++)
            {
                int rho = (int) round(cx * cos_table_data[theta_index] + cy * sin_table_data[theta_index] + (npy_float64) (distance / (npy_float64) 2.0));
                accumulator_data[(rho * thetas_size) + theta_index - 1] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
                accumulator_data[(rho * thetas_size) + theta_index] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
                accumulator_data[(rho * thetas_size) + theta_index + 1] += !use_reference ? 1 : 255 - (*(npy_uint8 *) PyArray_GETPTR2(reference_image_array, y, x));
            }
...
static PyObject* calc_threshold_by_otsu(PyObject* self, PyObject* args, PyObject *keywds)
{
	...
	for (int t = 0; t < 256; t++)
	{
	    ...
	    n1 = PyArray_Sum((PyArrayObject *) class1, NPY_MAXDIMS, PyArray_DESCR((PyArrayObject *) class1)->type_num, NULL);
	    n2 = PyArray_Sum((PyArrayObject *) class2, NPY_MAXDIMS, PyArray_DESCR((PyArrayObject *) class2)->type_num, NULL);
...
static PyMethodDef myimage_module_methods[] = {
    {
        "hough_transform_center",
        (PyCFunction) hough_transform_center,
         METH_VARARGS | METH_KEYWORDS,
         "Hough transform Center\n"
         "  Args:\n"
         "      image(ndarray): Input image.\n"
         "      reference_image(ndarray): Image referenced to luminance value.\n"
         "      theta_range(tuple): Range of theta to be calculated.\n"
         "      multi_vote(boolean): Vote for more than one rho.\n"
         "  Returns:\n"
