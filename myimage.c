...
static PyObject *hough_transform(PyObject *self, PyObject *args, PyObject *keywds) {
    ...
    if (!PyArg_ParseTupleAndKeywords(args, keywds, "O|OOp", kwlist, &image, &reference_image, &theta_range,
                                     &multi_vote)) {
    ...
    if (multi_vote) {
        for (int i = 0; i < x_indexes_size; i++) {
            ...
            for (int theta_index = 0; theta_index < thetas_size; theta_index++) {
                int rho = (int) round(
                        x * cos_table_data[theta_index] + y * sin_table_data[theta_index] + (npy_float64) distance);
                accumulator_data[(rho * thetas_size) + theta_index - 1] += !use_reference ? 1 : 255 -
...
static PyObject *hough_transform_center(PyObject *self, PyObject *args, PyObject *keywds) {
    ...
    if (multi_vote) {
        for (int i = 0; i < x_indexes_size; i++) {
            ...
            for (int theta_index = 0; theta_index < thetas_size; theta_index++) {
                int rho = (int) round(cx * cos_table_data[theta_index] + cy * sin_table_data[theta_index] +
                                      (npy_float64)(distance / (npy_float64) 2.0));
                accumulator_data[(rho * thetas_size) + theta_index - 1] += !use_reference ? 1 : 255 -
                                                                                                (*(npy_uint8 *) PyArray_GETPTR2(
                                                                                                        reference_image_array,
                                                                                                        y, x));
